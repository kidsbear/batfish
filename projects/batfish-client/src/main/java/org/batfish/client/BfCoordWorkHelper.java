...
public class BfCoordWorkHelper {
  ...
  public WorkItem getWorkItemParse(String containerName, String testrigName, boolean doDelta) {
    WorkItem wItem = new WorkItem(containerName, testrigName);
  ...
  public String initContainer(@Nullable String containerName, @Nullable String containerPrefix) {
    ...
    try {
      WebTarget webTarget = getTargetV2(Lists.newArrayList(CoordConsts.SVC_KEY_CONTAINER_NAME));
      ...
      if (response.getStatus() != Status.CREATED.getStatusCode()) {
        _logger.error("initContainer: Did not get the expected response\n");
        _logger.error(response.readEntity(String.class) + "\n");
        return null;
      }
